...
def raise_mr(ticket_key: str, fix: dict, client: GitLabClient,
    ...
    `targets` optionally maps `project -> target_branch`, letting the reviewer
    choose which branch each repo's MR merges into (and which ref the fix branch is
    cut from + re-applied against). A repo absent from the map falls back to
    `client.default_ref(project)` — the integration branch, the established default."""
    cfg = _write_cfg()
    if not cfg:
    ...
    with httpx.Client(base_url=base + "/api/v4",
        ...
        for project, files in by_project.items():
            ...
            # The branch the MR merges into: reviewer's choice, else the integration
            # default. Cut the fix branch from it AND re-apply edits against it, so the
            # exact-match apply is checked against the exact content we'll merge into.
            target = (targets.get(project) or "").strip() or client.default_ref(project)

            # Rebuild each file's new content from CURRENT source + the reviewed edits.
            actions, skipped = [], []
            for path, edits in files:
                try:
                    raw = client.get_file(project, target, path)
            ...
            # 1) branch off the chosen target (ok if it already exists)
            rb = w.post(f"/projects/{pid}/repository/branches",
                        params={"branch": branch, "ref": target})
